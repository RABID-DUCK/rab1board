  ...
  window.Echo["private"]('desks').listen('.desk_move', function (res) {
    ...
    fetch('/api/column/getDesks', {
      ...
      if (data.length > 1) {
        ...
        var oldDesks = document.querySelectorAll("[data-desk-id='".concat(data.id, "']"));
        oldDesks.forEach(function (item2) {
